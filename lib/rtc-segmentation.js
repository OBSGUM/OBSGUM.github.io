...
// rtcButton.addEventListener("click", init);
    ...
    async function createSegmentOffer(i) {
      ...
      const offer = await segmentPeerConnections[i].createOffer();
      await segmentPeerConnections[i].setLocalDescription(offer);
      ...
      await obs.on("CustomEvent", async function (event) {
        ...
        if (event.event_name === `rtc-answer-${segmentID}_${i}`) {
          ...
          if (!segmentPeerConnections[i].currentRemoteDescription) {
            await segmentPeerConnections[i].setRemoteDescription(answer);
    ...
    async function rtcSegmentConnectionComplete(i) {
      ...
      segmentPeerIndex++
     console.log(`create a PC if ${segmentPeerIndex} < ${numberOfPeerConnections} `, segmentPeerIndex < numberOfPeerConnections)
     if(segmentPeerIndex < numberOfPeerConnections){
